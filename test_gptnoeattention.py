...
def benchmark_dp(rank, args, world_size):
    ...
    model = gpt2warpper(copy.deepcopy(model), split=2)
    ...
    inputs.data.div_(1000)
    with torch.no_grad():
        for epoch in range(num_epochs):
            start_time = time.time()
            outputs = attention(hidden_states=inputs)
            outputs_ref = attention_ref(hidden_states=inputs)

            
            # # output_parallel.mean().backward()
            assert torch.allclose(outputs_ref[0], outputs[0], atol=1e-1), f"{torch.max(outputs_ref[0]-outputs[0]), outputs_ref[0],outputs[0]}"
